...
contract PleaseBuy is Context, IERC20, Ownable { 
    ...
    uint256 private _previousLiquidityFee = _liquityFee; 
   ...
   //Sets Processing Tokens - Set up
    ...
    function setEnableFeeSwap(bool true_or_false) external onlyOwner {
    ...
    */
    ...
    function removeAllFee() private {
        if(_TotalFee == 0 && _buyFee == 0 && _sellFee == 0 && _devFee == 0 && _teamFee == 0 && _burnFee == 0 && _liquityFee == 0 ) return;
        _previousBuyFee = _buyFee; 
        _previousSellFee = _sellFee; 
        _previousLiquidityFee = _liquityFee;
    ...
    function restoreAllFee() private {
        ...
        _liquityFee = _previousLiquidityFee;
    ...
    function _transfer(
        ...
        */
        ...
        takeFee = false;
         _liquityFee = _sellFee+_devFee+_buyFee;
         _TotalFee = _liquityFee+_burnFee+_teamFee;   
        ...
        if (from == uniswapV2Pair && ItsActivedFees[uniswapV2Pair] == true){
              takeFee = true;
            _liquityFee = _buyFee+_devFee;
            _TotalFee = _liquityFee+_burnFee+_teamFee;
         }

        if (to == uniswapV2Pair && ItsActivedFees[uniswapV2Pair] == true){
              takeFee = true;
            _liquityFee = _sellFee+_devFee;
            _TotalFee = _liquityFee+_burnFee+_teamFee;
        }
    ...
    // Processing tokens from contract
     function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 denominator = (_devFee + _buyFee + _sellFee) * 2; 
        uint256 liquity = _buyFee + _sellFee; //4
        uint256 tokensToAddLiquidityWith = contractTokenBalance * liquity / denominator;
        ...
        uint256 unitBalance = deltaBalance / (denominator - liquity);
        uint256 bnbToAddLiquidityWith = unitBalance * liquity;
        ...
        uint256 developmentAmt = unitBalance * 2 * _devFee;
        if(developmentAmt > 0){
          payable(wallet_Dev).transfer(developmentAmt);
        }
    ...
    private {(uint256 tTransferAmount, uint256 tDev, uint256 tBurn, uint256 tTeam) = _getValues(tAmount);
        ...
        _tOwned[wallet_team] = _tOwned[wallet_team].add(tTeam);   
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 tDev = tAmount*_liquityFee/100;
